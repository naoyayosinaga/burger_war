...
FRAME_WIDTH = 250  # Resized frame width
FRAME_HEIGHT = 250  # Resized frame height
...
class Agent():
    ...
    def build_network(self):
        ...
        model.add(Conv2D(32, (7,7), strides=(3,3), activation='relu', input_shape=(FRAME_WIDTH, FRAME_HEIGHT,STATE_LENGTH)))
        model.add(Conv2D(32, (5,5), strides=(1, 1), activation='relu'))
        model.add(Conv2D(16, (1,1), strides=(1, 1), activation='relu'))
...
#input ,,rx,ry,ex,ey
def reverse_state_init(mat,rx,ry,ex,ey,e_theta,point_list):


    observation2 =  lim.learn_image_maker(ex,ey,e_theta,rx,ry,point_list[0],point_list[1],point_list[2],point_list[3],point_list[4],point_list[5],point_list[6],point_list[7],point_list[8],point_list[9],point_list[10],point_list[11],point_list[12],point_list[13],point_list[14],point_list[15],point_list[16],point_list[17])

    observation2 = cv2.warpAffine(observation2,mat,(250,250))
    
    reverse_state = [observation2 for _ in range(STATE_LENGTH)]
    
     
    return np.array(reverse_state)
        
def reverse_state_update(state,mat,rx,ry,ex,ey,e_theta,point_list):
...
def main():
    ...
    if TRAIN:  # Train mode
        for _ in range(NUM_EPISODES):
            ...
            e_action = 0
            mat = cv2.getRotationMatrix2D((125,125),180,1.0)
            ...
            if  is_init_state_random <=7:
                ...
                is_init_state_random = random.randint(1,10)
                if  is_init_state_random <=3:
                     ex = random.randint(20,230)
                     ey = random.randint(20,230)
                     while(simulator.collision_check(ex,ey,rx,ry)):
                          ex = random.randint(20,230)
                          ey = random.randint(20,230)
            ...
            reverse_state = reverse_state_init(mat,rx,ry,ex,ey,e_theta,point_list)
            reverse_state = reverse_state.transpose(1,2,0)
            
            while not terminal:
                last_observation = observation
                action = agent.get_action(state)+1
                e_action = agent.get_action(reverse_state)+1
                #observation, reward, terminal, _ = env.step(action)
                reward,terminal,point_list,time,rx,ry,r_theta,ex,ey,e_theta,point_list,reward_circle_list=simulator.run(time,action,e_action,rx,ry,r_theta,ex,ey,e_theta,point_list,reward_circle_list)
                observation = lim.learn_image_maker(rx,ry,r_theta,ex,ey,point_list[0],point_list[1],point_list[2],point_list[3],point_list[4],point_list[5],point_list[6],point_list[7],point_list[8],point_list[9],point_list[10],point_list[11],point_list[12],point_list[13],point_list[14],point_list[15],point_list[16],point_list[17])
                # env.render()
                #processed_observation = preprocess(observation, last_observation)
                state = agent.run(state, action, reward, terminal, observation)
                reverse_state = reverse_state_update(reverse_state,mat,rx,ry,ex,ey,e_theta,point_list)
                print((rx,ry,r_theta))
